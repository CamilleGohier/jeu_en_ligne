...
export class Toolbar {
    ...
    createPrimaryToolbar() {
        ...
        tools.forEach((tool, index) => {
            ...
            if (tool.secondary && tool.secondary.some((icon => icon.key))) {
                ...
                if (dictionary[iconSecondaryInPrimaryToolbar] && dictionary[iconSecondaryInPrimaryToolbar].origin) {
                    icon.secondarySprite.setOrigin(dictionary[iconSecondaryInPrimaryToolbar].origin[0] + 0.5, dictionary[iconSecondaryInPrimaryToolbar].origin[1] + 0.25);
                }
            }
    ...
    updateSecondaryToolbar() {
        ...
        if (selectedTool.secondary && selectedTool.secondary.some((item) => item.key)) {
            ...
            selectedTool.secondary.forEach((option, index) => {
                ...
                if (dictionary[option.key] && dictionary[option.key].origin) {
                    icon.setOrigin(dictionary[option.key].origin[0] +0.5, dictionary[option.key].origin[1] +0.25);
                }
            })
    ...
    updateSecondaryIconInPrimaryToolbar() {
        ...
        if (selectedTool.secondary && selectedTool.secondary.some((item) => item.key)) {
            const secondarySprite = this.primaryItems[this.selectedTool].secondarySprite;

            if (secondarySprite) {
                secondarySprite.setTexture(this.secondaryToolbar.icons[this.selectedSecondary].secondaryData.key || 'empty');
            }
            else {
                const primaryIcon = this.primaryItems[this.selectedTool];
            ...
            if (dictionary[this.secondaryToolbar.icons[this.selectedSecondary].secondaryData.key] && dictionary[this.secondaryToolbar.icons[this.selectedSecondary].secondaryData.key].origin) {
                secondarySprite.setOrigin(dictionary[this.secondaryToolbar.icons[this.selectedSecondary].secondaryData.key].origin[0] +0.5, dictionary[this.secondaryToolbar.icons[this.selectedSecondary].secondaryData.key].origin[1] +0.25);
            }
            else {
                secondarySprite.setOrigin(0.5);
            }
        }
    }
